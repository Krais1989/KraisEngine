...
#define KE_CRITICAL(...) KE::Log::s_clientLogger->critical(__VA_ARGS__)
	...
	#define KE_CORE_ASSERT(cond, ...) { if (!(cond)) {KE_CORE_ERROR("CORE ASSERT FAILED: {0}", __VA_ARGS__); __debugbreak();} }
	#define KE_ASSERT(cond, ...) { if (!(cond)) {KE_ERROR("ASSERT FAILED: {0}", __VA_ARGS__); __debugbreak();} }
#else
	#define KE_CORE_ASSERT(cond, ...)
	#define KE_ASSERT(cond, ...)
#endif
